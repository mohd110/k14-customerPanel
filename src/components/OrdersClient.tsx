        ...
        return (
          <div key={step.status} className="flex-1 flex flex-col items-center relative">
            {i < STATUS_STEPS.length - 1 && (
              <div className={`absolute top-4 left-1/2 w-full h-0.5 ${i < current ? 'bg-[#e23744]' : 'bg-[#2a2a2a]'}`} />
            )}
            <div className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center border-2 transition-colors ${
              done ? 'bg-[#e23744] border-[#e23744]' : 'bg-neutral-900 border-[#2a2a2a]'
            }`}>
  ...
  return (
    <div className="space-y-3">
      {orders.map((order) => {
        ...
        return (
          <div key={order.id} className="bg-neutral-900 rounded-xl overflow-hidden" style={{ boxShadow: '0 2px 8px rgba(45,52,54,0.06)' }}>
            {/* Header */}
            <div className="px-4 pt-4 flex items-start justify-between">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                isCancelled ? 'bg-[#2a1416] text-[#e23744]' :
            ...
            {/* Items */}
            <div className="px-4 pb-3 border-t border-[#1c1c1c] pt-3">
              <p className="text-xs text-[#b5b5b5] mb-1.5">
